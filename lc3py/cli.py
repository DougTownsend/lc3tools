...
def lc3pysim():
    try:
        ...
        print("Error: lc3pysim requires SDL2 to be installed.",
              file=sys.stderr)
        if sys.platform == "darwin":
            print("  Install it with: brew install sdl2 pkg-config", file=sys.stderr)
            ...
            print("  Ubuntu/Debian: sudo apt install libsdl2-dev", file=sys.stderr)
            print("  Then rebuild:  pip3 install . --no-build-isolation --no-cache-dir", file=sys.stderr)
        sys.exit(1)
